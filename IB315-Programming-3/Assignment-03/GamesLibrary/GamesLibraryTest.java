...
public class GamesLibraryTest {
  ...
  public void test_getGenreCount(){
    Map<Genre, Long> genreCount = lib.getGenreCount();

    assertEquals((long) genreCount.get(Genre.ACTION), 47);
    assertEquals((long) genreCount.get(Genre.METROIDVANIA), 2);
    assertEquals((long) genreCount.get(Genre.ROLE_PLAYING_GAME), 14);
  ...
  @Test
  public void test_getAverageMetacriticScore(){
    assertEquals(92.4848,lib.getAverageMetacriticScore(), 1e-3);
  }

  @Test
  public void test_getGamesForGenre(){
    List<Game> games = lib.getGamesforGenre(Genre.ACTION);

    assertTrue(
        lib.getGamesReadOnly().stream().allMatch(
          game -> game.getGenre() == Genre.ACTION || !games.contains(game)));
  }

  @Test
  public void test_gameReleasedBetween(){
    assertTrue(lib.gameReleasedBetween(2006, 2010));
    assertTrue(lib.gameReleasedBetween(2005, 2005));
    assertFalse(lib.gameReleasedBetween(1990, 1995));
    assertFalse(lib.gameReleasedBetween(1990, 1990));
  }

  @Test
  public void test_getGenresWithGames(){
    GamesLibrary lib = new GamesLibrary();
    lib.addPlatform(ns);
    lib.addPlatform(pc);

    lib.addGame( new Game("The Legend of Zelda: Breath of the Wild", Genre.ADVENTURE, ns, 2017, 97) );
    lib.addGame( new Game("Super Mario Odyssey", Genre.PLATFORMER, ns, 2017, 97) );
    lib.addGame( new Game("Divinity: Original Sin II - Definitive Edition", Genre.ROLE_PLAYING_GAME, ns, 2019, 93) );

    lib.addGame(new Game("Grand Theft Auto V", Genre.ACTION, pc, 2015, 96));
    lib.addGame(new Game("The Orange Box", Genre.SHOOTER, pc, 2007, 96));
    lib.addGame(new Game("Baldur's Gate II: Shadows of Amn", Genre.ROLE_PLAYING_GAME, pc, 2000, 95));

    List<Genre> genres = lib.getGenresWithGames();

    assertTrue(genres.contains(Genre.ADVENTURE));
    assertTrue(genres.contains(Genre.PLATFORMER));
    assertTrue(genres.contains(Genre.ROLE_PLAYING_GAME));
    assertTrue(genres.contains(Genre.ACTION));
    assertTrue(genres.contains(Genre.SHOOTER));

    assertFalse(genres.contains(Genre.BUILDING));
    assertFalse(genres.contains(Genre.METROIDVANIA));
    assertFalse(genres.contains(Genre.PUZZLE));
    assertFalse(genres.contains(Genre.SPORT));
    assertFalse(genres.contains(Genre.STRATEGY));
  }
  ...
  public void test_topGames(){
    ...
    Game zelda = new Game("The Legend of Zelda: Breath of the Wild", Genre.ADVENTURE, ns, 2017, 98);
    Game mario =  new Game("Super Mario Odyssey", Genre.PLATFORMER, ns, 2017, 97);
    Game divinity = new Game("Divinity: Original Sin II - Definitive Edition", Genre.ROLE_PLAYING_GAME, ns, 2019, 93) ;
    Game gta5 = new Game("Grand Theft Auto V", Genre.ACTION, pc, 2015, 94);
    Game orangeBox = new Game("The Orange Box", Genre.SHOOTER, pc, 2007, 96);
    Game bg2 = new Game("Baldur's Gate II: Shadows of Amn", Genre.ROLE_PLAYING_GAME, pc, 2000, 93);

    lib.addGame(zelda);
    lib.addGame(mario);
    lib.addGame(divinity);
    lib.addGame(gta5);
    lib.addGame(orangeBox);
    lib.addGame(bg2);

    List<Game> top3 = lib.topGames(3);

    assertEquals(top3.size(), 3);
    assertEquals(top3.get(0), zelda);
    assertEquals(top3.get(1), mario);
    assertEquals(top3.get(2), orangeBox);;
  }

  @Test
  public void test_topGamesToFew(){
    GamesLibrary lib = new GamesLibrary();
    lib.addPlatform(ns);
    lib.addPlatform(pc);
    ...
    Game gta5 = new Game("Grand Theft Auto V", Genre.ACTION, pc, 2015, 94);
    Game orangeBox = new Game("The Orange Box", Genre.SHOOTER, pc, 2007, 96);
    Game bg2 = new Game("Baldur's Gate II: Shadows of Amn", Genre.ROLE_PLAYING_GAME, pc, 2000, 95);
    ...
    List<Game> top10 = lib.topGames(10);

    assertEquals(top10.size(), 6);
    assertEquals(top10.get(0), zelda);
    assertEquals(top10.get(1), mario);
    assertEquals(top10.get(2), orangeBox);;
    assertEquals(top10.get(3), bg2);;
    assertEquals(top10.get(4), gta5);;
    assertEquals(top10.get(5), divinity);;
  }
  
  @Test
  public void test_getTopTenListString(){
    GamesLibrary lib = new GamesLibrary();
    lib.addPlatform(ns);
    lib.addPlatform(pc);

    lib.addGame(new Game("The Legend of Zelda: Breath of the Wild", Genre.ADVENTURE, ns, 2017, 98));
    lib.addGame(new Game("Super Mario Odyssey", Genre.PLATFORMER, ns, 2017, 97));
    lib.addGame(new Game("Grand Theft Auto V", Genre.ACTION, pc, 2015, 94));
    lib.addGame(new Game("The Orange Box", Genre.SHOOTER, pc, 2007, 96));

    assertEquals("The Legend of Zelda: Breath of the Wild, Super Mario Odyssey, The Orange Box, Grand Theft Auto V",
        lib.getTopTenListString());
