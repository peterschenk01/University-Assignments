...
class PGAgent(nn.Module):
        ...
        if self.critic is None:
            ...
            if self.gae_lambda is None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    advantages[i] = delta + ((1 - terminals[i]) * (self.gamma * self.gae_lambda * advantages[i + 1]))
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        discounts = np.power(self.gamma, np.arange(len_rewards))
        discounted_rewards = discounts * rewards
        sum = np.sum(discounted_rewards)
        
        discounted_return = np.full(len_rewards, sum)
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for i in range(len_rewards):
            arange_len_rewards = np.arange(len_rewards)
            discounts = np.power(self.gamma, (arange_len_rewards[i:] - i))
            discounted_rewards = discounts * rewards[i:]

            sum = np.sum(discounted_rewards)

            discounted_reward_to_go.append(sum)
